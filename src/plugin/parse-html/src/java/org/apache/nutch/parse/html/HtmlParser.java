...
public class HtmlParser implements Parser {
  ...
  private Document parseDoc(InputSource input) throws Exception {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String url = "http://www.youku.com/show_page/id_z2cff9ca02fa111e29013.html";
    File file = new File(name);
    byte[] bytes = new byte[(int)file.length()];
    DataInputStream in = new DataInputStream(new FileInputStream(file));
    in.readFully(bytes);
    Configuration conf = NutchConfiguration.create();
	conf.set("plugin.includes","parse-video");
    HtmlParser parser = new HtmlParser();
    parser.setConf(conf);
    Parse parse = parser.getParse(
            new Content(url, url, bytes, "text/html", new Metadata(), conf)).get(url);
    //System.out.println("data: "+parse.getData());
   /* File logf = new File("zhous.log");
    if (!logf.exists()) {  
        logf.createNewFile();  
      }  
    FileWriter resultFile = new FileWriter(logf);  
      PrintWriter myFile = new PrintWriter(resultFile);
myFile.println("text:"+parse.getText());
resultFile.close();  */
  ...
  public void setConf(Configuration conf) {
    ...
    this.defaultCharEncoding = getConf().get(
     ...
     //   "parser.character.encoding.default", "gb2312");
